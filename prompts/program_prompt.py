...
These questions are MULTIPLE CHOICE or TRUE/FALSE. Your program MUST store the answer letter (A, B, C, or D) in the variable "final_result".
...
Example 3:
Question: "Which option correctly describes the next step in the assembly?
Options:
  A. Add a red brick on top of the blue brick
  B. Add a yellow brick to the left side
  C. Remove the green brick
  D. Rotate the top brick 90 degrees"
Solution:
1) Analyze the current state of the assembly and determine which textual option best matches the next logical step.
...
Solution:
1) Use VQA holistically on the current image.
2) Ask the vision backend to determine the height relationship.
...
IMPORTANT RULES:
1) This is a MULTIPLE CHOICE or TRUE/FALSE question. Your final_result MUST be a single option letter (A, B, C, or D).
...
7) Do NOT write programs for image generation, image editing, or any task where the desired output is a new image.
8) For HEIGHT comparison questions ("which is higher/shorter/taller in 3D space"), ALWAYS use holistic VQA: vqa(image, full_question_with_all_options, None). The LEGO images already have colored markers (red/blue rectangles, arrows) that the vision backend can inspect directly. Do NOT use loc() or compare_3D_heights for height questions — just pass the entire question text with options to vqa() with bbox=None.
